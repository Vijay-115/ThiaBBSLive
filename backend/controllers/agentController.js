...
function extractAddressBack(rawText) {
  ...
  // 8) Street = everything before city/state
...
async function pdfFirstPageToImage(pdfPath) {
  ...
  try {
    ...
    throw new Error(
      "PDF OCR failed. Check Poppler install and POPPLER_PATH. Details: " +
        err.message
    );
  }
...
/* ------------------------------ Controllers --------------------------- */
// POST /api/agent/ocr  (field: document)  ?side=aadhaar_front|aadhaar_back|gst
...
// Legacy: PATCH /api/agent/:vendorId/step
...
// PUT /api/agent/gst  (multipart form-data: document + fields)
exports.updateGst = async (req, res) => {
  try {
    ...
    // accept nested form-data e.g., gst_address[city]
    for (const k of [
      "floorNo",
      "buildingNo",
      "street",
...
exports.registerAgent = async (req, res) => {
  try {
    const doc = new Agent(req.body);
    await doc.save();
    res
      .status(201)
      .json({ message: "Agent registered successfully", agent: doc });
  } catch (error) {
...
exports.registerVendor = exports.registerAgent;
